...
const { getCustomRoute } = require('../helpers/resolve-custom-route')
...
module.exports = class FileEntryPlugin extends Tapable {
  ...
  getEntryPackages (entry) {
    ...
    // 读取options中的route与entry(app.json)合并
    let entryData
    let entryJSON = JSON.parse(
      readFileSync(entry, { encoding: 'utf8' })
    )
    if(this.options.route){
      let routeJSON = getCustomRoute(this.options.route)
      entryJSON.pages = routeJSON.pages || []
      entryJSON.subPackages = routeJSON.subPackages || []
      if(routeJSON.tabbarList.length > 0){
        entryJSON.tabBar = entryJSON.tabBar || {}
        entryJSON.tabBar.list = routeJSON.tabbarList || []
      }
      entryData = entryJSON
    } else {
      entryData = entryJSON
